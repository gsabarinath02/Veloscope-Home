...
- Voice: one-time conversation mode opens the microphone after browser permission, detects speech and silence automatically, sends captured segments to Deepgram Speech-to-Text, and resumes listening after the assistant speaks. Voice replies use Deepgram TTS through `/api/speak`, with browser speech synthesis as a fallback.
